...
class TagsView extends VuexModule implements ITagsViewState {
  ...
  private DEL_VISITED_VIEW(view: ITagView) {
    if(this.visitedViews.length ===1 && this.visitedViews[0].title !== '欢迎'){
      Message({
        message: '将跳转到欢迎页面了',
        type: 'warning'
      });
      return this.visitedViews = [{'fullPath':'/main','meta':{isChecking:true,title:'欢迎'},name:'welcome',path:'/main',title:'欢迎'}];
    }else if(this.visitedViews.length ===1 && this.visitedViews[0].title === '欢迎'){
      Message({
        message: '已经是欢迎页面了',
        type: 'warning'
      });
